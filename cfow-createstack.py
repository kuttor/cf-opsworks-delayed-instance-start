...
"""
Datalake one-button environment automater

Usage:
  datalake-main.py -h | --help
  datalake-main.py --version
  datalake-main.py --env <env>
...
# creates lambda for dead letter queue
def create_function():
    response = boto3.client('lambda')
    response = client.create_function(
        FunctionName='workflow-ancestry-forwarder-' + env,
        Runtime='python2.7',
        Role='lambda-dynamo-trigger',
        ...
        Description='Sends ancestry messages to dli inbox queue',
        Timeout=3,
        MemorySize=128,
        Publish=True,
        VpcConfig={
            'SubnetIds': ['subnet-a', 'subnet-b', 'subnet-c'],
            'SecurityGroupIds': ['sg-some_sg']
        }
    )
